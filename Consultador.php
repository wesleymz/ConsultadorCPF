...
class Consultador {
    ...
    public static function getHtmlCpf($cpf, $data_nasc, $captcha) {
        ...
        curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
        curl_setopt($ch, CURLOPT_MAXREDIRS, 3);
        curl_setopt($ch, CURLOPT_REFERER, "https://www.receita.fazenda.gov.br/Aplicacoes/SSL/ATCTA/cpf/ConsultaPublica.asp?Error=1&CPF=" . $cpf . "&NASCIMENTO=" . $data_nasc);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
    ...
    public static function extrairNomeEmpresarial($html) {
        ...
        if (strpos($html, "NOME EMPRESARIAL ") !== false) {
            $captura = array();
            preg_match("/(<font face=\"Arial\" style=\"font-size: 6pt\">\s*NOME EMPRESARIAL.*?<b>)(.*?)(<\/b>)/s", $html, $captura);
            $output["nome"] = $captura[2];
            return $output;
        } else {
    ...
    public static function extrairNomePessoaFisica($html) {
        ...
        if (strpos($html, "Nome da Pessoa") !== false) {
            $captura = array();
            preg_match("/(<span class=\"clConteudoDados\">Nome da Pessoa F.sica: <b>)(.*?)(<\/b>)/s", $html, $captura);
            $output["nome"] = $captura[2];
            return $output;
        } else {
